...
def parse_str(p, brands):
    return sorted(r_csv(p, lambda r: {
    ...
    } if r["Keyword"] and not any(b in r["Keyword"].lower() for b in brands) else None), key=lambda x: x["search_volume"], reverse=True)[:10]

top_dc = parse_str("data/5_dealcheck_organic_positions.csv", ["dealcheck"])
top_br = parse_str("data/6_bricked_organic_positions.csv", ["bricked", "brikd", "brico"])
top_hc = parse_str("data/9_housecanary_organic_positions.csv", ["housecanary", "house canary", "housecanery", "comehome", "withroam"])
...
def parse_gap(p):
    return sorted(r_csv(p, lambda r: {
        "keyword": r["Keyword"], "search_volume": int(r.get("Volume") or 0), "keyword_difficulty": int(r.get("Keyword Difficulty") or 0), "cpc": float(r.get("CPC") or 0.0)
    } if r["Keyword"] else None), key=lambda x: x["search_volume"], reverse=True)[:10]

top_br_gaps = parse_gap("data/7_bricked_keyword_gap.csv")
top_hc_gaps = parse_gap("data/10_housecanary_keyword_gap.csv")
